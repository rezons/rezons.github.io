...
function Cols.new(lst,       self,now) 
  ...
  for k,v in pairs(lst) do
    ...
    if not v:find"?" then 
      ...
      push(nump(v) and self.ys or self.xs, now) end end
  return self end
...
function Sample:div(rows,top)
  rows = rows or shuffle(self.rows)
  top  = top or #rows  
  some = {}
  for i= top,top-some,-1 do push(some, rows[i]) end
  table.sort(some, function(x,y) return self:better(x,y) end)
  for i= top-some-1,1,-1 do
    row1, d, j = rows[i], 1E32, some // 2
    for k,row2 in pairs(some) do
      tmp = self:dist(row1,row2) 
      if tmp<d then d,j = tmp,k end end end 
end

-- Main
function main(file,     s)
  s = Sample.new()
...
for k,v in pairs(_ENV) do if not b4[k] and type(v)~="function" then print("? ",k,type(v)) end end
