...
function evals(task,out)
  local x,w,y,d,p,any,mre,eval,want,got
  d = d or 2
  function p(x)      return round(x,d) end
  function any(_,x)  return p(rand(x[1],x[2])) end
  function eval(_,f) return p(f(x,w)) end
  for i=1,task.n do 
    x = sort(map(task.x,any))
    w = sort(map(task.w,any))
    y = sort(map(task.y, eval))
    want=task.truth.y1(x)
    push(out, {abs((y.y1 - want)/want), w}) end 
  return out end
